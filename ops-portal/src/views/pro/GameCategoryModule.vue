<template>
  <div class="pro-page">
    <el-card>
      <template #header>
        <div class="head-row">
          <span>{{ lt('遊戲分類管理', '遊戲分類管理', 'Game Category Management') }}</span>
          <div class="actions">
      ...
      <el-table :data="categories" v-loading="loading">
        ...
        <el-table-column prop="name" :label="lt('分類名稱', '分類名稱', 'Category Name')" min-width="220" />
...
import {
  ...
  createOpsGameCategory,
  updateOpsGameCategory,
  deleteOpsGameCategory
} from '../../api'
...
const loadCategories = async () => {
  ...
  try {
    ...
    ElMessage.error(error.message || lt('加載遊戲分類失敗', '加載遊戲分類失敗', 'Failed to load game categories'))
  } finally {
    loading.value = false
  }
}
...
const confirmEdit = async () => {
  ...
  try {
    ...
    const res = editingId.value === null
      ? await createOpsGameCategory(payload)
      : await updateOpsGameCategory(editingId.value, payload)
    categories.value = res.data || []
...
const removeCategory = async (row) => {
  try {
    ...
    const res = await deleteOpsGameCategory(row.id)
    categories.value = res.data || []
